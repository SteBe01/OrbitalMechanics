...
for i=1:len-1
    ...
    if split
        last = i;
        plot(lon(first:last),lat(first:last), LineWidth=1.5, Color="blu", HandleVisibility="off")
        first = i + 1;
    end
end
plot(lon(first:end),lat(first:end), LineWidth=1.5, Color="blu", HandleVisibility="off")
